...
function buildMap(input, radius) {
  const competitors = buildCompetitors(input, radius);
  const competition = CATEGORY_COMPETITION[input.category] ?? 45;
  const density = clamp(Math.round(competition / 10), 1, 10);
  const avgDistance = competitors.reduce((sum, c) => sum + c.distanceKm, 0) / Math.max(competitors.length, 1);
  const nearest = [...competitors].sort((a, b) => a.distanceKm - b.distanceKm).slice(0, 3);
  return {
...
function buildScores(input) {
  ...
  return {
    ...
    supplyGap: {
      ...
      reasoning: "Supply gap is calculated as the difference between estimated demand and the capacity of existing suppliers and cooperatives in the area.",
    ...
    competition: {
      ...
      reasoning: "Competition is scored from the number of nearby suppliers, their price ranges, and how concentrated they are around the main market cluster.",
      provenance: "Observed",
      confidence: "Medium"
    },
    opportunity: {
...
function buildSwot(input) {
  ...
  return {
    ...
    threats: [
      competition >= 60 ? "Several established suppliers may respond with price cuts." : "New entrants could increase competition as the market grows.",
...
function buildRisk(input) {
  ...
  const categories = [
    {
      category: "Financial",
      level: riskLevel(60 + (input.capital > 2e5 ? -10 : 10)),
...
function buildMarket(input, radius) {
  const demand = CATEGORY_DEMAND[input.category] ?? 65;
  const competition = CATEGORY_COMPETITION[input.category] ?? 45;
  ...
  return {
    ...
    accessibility: radius === "5km" ? "Good road access within 5 km; last-mile village roads are partly unpaved." : "Wider reach within 10 km; longer routes increase transport time and cost.",
    ...
    competitionLevel: competition >= 60 ? "High" : competition >= 40 ? "Moderate" : "Low"
  };
}
function generateDemoAnalysis(input, radius) {
  const result = {
